...
while ($row = $stmt->fetch()) {
    ...
    echo "<article class='post' style='margin-bottom:60px; opacity:0.8;'>
            <h1 class='title' style='color:#888;'>/ " . safe($row['title']) . " " . ($row['file_path'] ? "📎" : "") . "</h1>";
    ...
    if (isset($_SESSION['auth'])) {
        echo "<div class='admin-links' style='margin-top:20px; border-top:1px solid #ccc; padding-top:10px;'>
                <a href='/?route=librares&act=del&id={$row['id']}&csrf_token={$_SESSION['csrf_token']}' style='color:red;' onclick='return confirm(\"УДАЛИТЬ ИЗ ТЛЕНА НАВСЕГДА?\")'>Удалить</a>
              </div>";
    }
