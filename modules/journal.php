...
if (isset($_SESSION['auth'])) {
    ...
    if (isset($_GET['act']) && $_GET['act'] == 'edit' && isset($_GET['del_file']) && $id > 0) {
        // CSRF ПРОВЕРКА ДЛЯ GET ЗАПРОСОВ (ПАРАНОЙЯ)
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_btn'])) {
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $allowed = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'zip', 'rar', '7z', 'pdf', 'txt'];
...
if (isset($_SESSION['auth']) && isset($_GET['act']) && ($_GET['act'] == 'add' || $_GET['act'] == 'edit')) {
    ...
    <form method='POST' enctype='multipart/form-data' style='width:100%; box-sizing:border-box;'>
        ".csrf_input()."
        <input type='text' name='title' value='".safe($p['title'])."' placeholder='ЗАГОЛОВОК' required style='width:100%; border:1px solid #000; padding:12px; margin-bottom:15px; box-sizing:border-box;'>
        <textarea name='content' placeholder='ПИШИ ТУТ СВОИ МЫСЛИ...' required style='width:100%; height:550px; border:1px solid #000; padding:15px; margin-bottom:20px; box-sizing:border-box; font-family:monospace; line-height:1.5;'>".safe($p['content'])."</textarea>
        <div style='border:1px solid #000; padding:20px; margin-bottom:25px; background:#fff;'><b>ВЛОЖЕНИЕ:</b> <input type='file' name='file'>";
        if (!empty($p['file'])) {
            echo "<br><br><small>ТЕКУЩИЙ ФАЙЛ: ".safe($p['file'])." <a href='/?route=journal&act=edit&id={$id}&del_file=1&csrf_token={$_SESSION['csrf_token']}' style='color:red;'>[ УДАЛИТЬ ]</a></small>";
        }
    echo "</div>
...
while ($row = $stmt->fetch()) {
    ...
    echo "<article class='post' style='margin-bottom:80px;'>
            <h1 class='title'>/ ".safe($row['title'])."</h1>
    ...
    if (isset($_SESSION['auth'])) {
        echo "<div class='admin-links' style='margin-top:25px; border-top:1px solid #ffb3d9; padding-top:10px;'>
                ...
                <a href='/?route=journal&act=del&id={$row['id']}&csrf_token={$_SESSION['csrf_token']}' onclick='return confirm(\"Стереть?\")'>Удалить</a>
              </div>";
    }
