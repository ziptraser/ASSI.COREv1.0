...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_comm_btn'])) {
    // В КРЫСУ НЕ ПРОЙДЕШЬ (CSRF)
    if (!csrf_verify($_POST['csrf_token'] ?? '')) { die("CSRF DETECTED"); }

    $topic_id = (int)$_POST['topic_id'];
    $name = trim($_POST['user_name']);
    $text = trim($_POST['comm_text']);
    ...
    if (empty($honeypot) && !$is_spam && !empty($name) && !empty($text) && $topic_id > 0) {
        $pdo->prepare("INSERT INTO lib_comments (topic_id, user_name, comment_text) VALUES (?, ?, ?)")
            ->execute([$topic_id, $name, $text]); // ФИКС: ДАННЫЕ ВЛЕТАЮТ В СЕЙФ
    }
...
if (isset($_SESSION['auth'])) {
    ...
    if (isset($_GET['act']) && $_GET['act'] == 'del_comm') {
        if (!csrf_verify($_GET['csrf_token'] ?? '')) { die("CSRF DETECTED"); }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['save_lib']))) {
        ...
        if (!empty($_FILES['lib_file']['name'])) {
            ...
            $allowed = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'zip', 'rar', '7z', 'pdf', 'txt', 'mp3', 'mp4'];
            ...
            if (in_array($ext, $allowed)) {
                ...
                die("FILE TYPE NOT ALLOWED. ПАРАНОЙЯ!");
            }
        }
        if ($id > 0) {
...
if (isset($_SESSION['auth']) && isset($_GET['act']) && ($_GET['act'] == 'add' || $_GET['act'] == 'edit')) {
    ...
    <form method='POST' enctype='multipart/form-data' style='width:100%; box-sizing:border-box;'>
        ".csrf_input()."
        <input type='text' name='title' value='".safe($p['title'])."' placeholder='НАЗВАНИЕ' required style='width:100%; border:1px solid #000; padding:12px; margin-bottom:15px; box-sizing:border-box;'>
        <textarea name='content' placeholder='ОПИСАНИЕ И ПОДРОБНОСТИ...' required style='width:100%; height:550px; border:1px solid #000; padding:15px; margin-bottom:20px; box-sizing:border-box; font-family:monospace; line-height:1.5;'>".safe($p['content'])."</textarea>
        <div style='border:1px solid #000; padding:20px; margin-bottom:25px; background:#fff;'><b>ФАЙЛ:</b> <input type='file' name='lib_file'>";
        if (!empty($p['file_path'])) { echo "<br><br><small>ТЕКУЩИЙ ФАЙЛ: ".safe($p['file_path'])."</small>"; }
    echo "</div>
...
while ($row = $topics->fetch()) {
            ...
            <div class='post-cut'>
                ...
                while($c = $comms->fetch()) {
                    echo "<div style='margin-bottom:15px; padding:10px; background:rgba(255,255,255,0.4); border-left:3px solid #ff007f;'>
                            <b style='font-size:0.8em; text-transform:uppercase;'>".safe($c['user_name']).":</b>
                            <div style='font-size:0.95em;'>".nl2br(safe($c['comment_text']))."</div>";
                    if (isset($_SESSION['auth'])) { echo "<a href='/?route=librares&act=del_comm&comm_id={$c['id']}&csrf_token={$_SESSION['csrf_token']}' style='font-size:0.75em; color:red; text-decoration:none;'>[ СТЕРЕТЬ ]</a>"; }
                    echo "</div>";
                }
                // ФОРМА КОММЕНТА
    ...
    if (isset($_SESSION['auth'])) {
        echo "<div class='admin-links' style='margin-top:25px; border-top:1px solid #ffb3d9; padding-top:10px;'>
                ...
                <a href='/?route=librares&act=to_archive&id={$row['id']}&csrf_token={$_SESSION['csrf_token']}'>В архив</a> | 
                <a href='/?route=librares&act=del&id={$row['id']}&csrf_token={$_SESSION['csrf_token']}' onclick='return confirm(\"СТЕРЕТЬ ПРОЕКТ?\")'>Удалить</a>
              </div>";
    }
